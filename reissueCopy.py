...
def main(x):
    os.system('cls')
    welcome.func()
    sid=input("Enter the book ID to be reissued")
    ...
    if flag==0:
        f.close()
        print("This book has never been issued or wrong book ID")
    ...
    g=open("C:/Users/Administrator/Documents/Python Scripts/Library/member_db1.txt")
    while True:
        w=g.readline()
        if(len(w)==0):
            break
        ...
        l.append(str(dic))
    g.close()
    h=open("C:/Users/Administrator/Documents/Python Scripts/Library/member_db1.txt","w")
    h.write(l[0])
    for i in l[1:]:
        h.write("\n"+i)
    h.close()
